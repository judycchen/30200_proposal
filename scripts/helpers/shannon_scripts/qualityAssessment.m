...
for k=1:size(data,2)  % data: TxR
    trace = data(:,k);  % TC of the current channel
    if ~any(isnan(trace))
        trace = zscore(trace);
        trace_orig = trace;
        ...
        lstMs = find(diff(bigchanges(:,1))==1);  % find where bigchanges increase by 1
        lstMf = find(diff(bigchanges(:,1))==-1);  % find where bigchanges decrease by 1
